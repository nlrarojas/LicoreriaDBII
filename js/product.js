...
$(document).ready( function() {    
    $.getJSON(uriGetUserRole).done(function (data) {      
        console.log(data);           
        writeTable(data.Products);
    });    
});

function writeTable(dataTable) {
    table = document.getElementById("tablePayments");
    var tableContent = "<thead class=thead-dark> <tr><th scope=col>Nombre</th><th scope=col>Tipo de añejado</th><th scope=col>Procedencia</th><th scope=col>Añod</th><th scope=col>Precio</th><th scope=col>Acciones</th></tr></thead>";    
    for (var i = 0; i < dataTable.length; i++){                
        ...
        tableContent += "<td>" + dataTable[i].nombreProcedencia + "</td>";
